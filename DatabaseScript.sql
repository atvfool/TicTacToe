...
IF EXISTS (SELECT 1 FROM sys.objects WHERE name = N'TGameDifficulties_TGames_FK')
BEGIN
	DROP FOREIGN KEY TGames_TGameDifficulties_FK
END
IF EXISTS (SELECT 1 FROM sys.objects WHERE name = N'TGameOutcome_TGames_FK')
BEGIN
	DROP FOREIGN KEY TGames_TGameOutcome_FK
END
IF EXISTS (SELECT 1 FROM sys.objects WHERE name = N'TGames_TGameMoves_FK')
BEGIN
	DROP FOREIGN KEY TGames_TGameMoves_FK
END
...
(
	  intGameID INT NOT NULL PRIMARY KEY
	, intMoveIndex INT NOT NULL PRIMARY KEY IDENTITY
	, intSquare INT
)
...
ALTER TABLE TGames ADD CONSTRAINT TGameDifficulties_TGames_FK
